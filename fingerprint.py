...
#########################根據實際HTTP流量，構建PDF矩陣
    def __init__(self, packet_limit=40, time_delta_bins=1001, max_packet_size=1500, max_time_delta=10**3):
    ...
    def train(self, csv_filepath):
        ...
        for trace in traffic_traces:
            ...
            for i, (size, time) in enumerate(trace):
                ...
                prev_time = time
                packet_stats[i][size][0] += 1  # 計數包大小
                packet_stats[i][size][1] += delta_t  # 累加時間間隔

        for packet_id in range(self.packet_limit): 
            pdf = np.zeros((self.max_packet_size - 40 + 1, self.time_delta_bins)) # 初始化一個pdf矩陣，形狀為(max_..., time_...)
            for size, (count, total_delta_t) in packet_stats[packet_id].items():
                avg_delta_t = total_delta_t / count if count > 0 else 0 # 計算平均時間間隔
                pdf[size - 40, int(np.log10(avg_delta_t) * 100)] = count # 將count填入pdf矩陣的對應位置
...
###########################################################################
    ...
    def smooth_pdf_matrix(self, pdf_matrix, sigma=0.8):
        window = np.outer(self.gaussian(51, sigma), self.gaussian(51, sigma)) # 計算二為高斯
